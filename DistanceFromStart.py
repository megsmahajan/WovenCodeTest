...
def find_smallest_dist(final_directions):
    ...
    if final_directions != "":
        ...
        print("Couldn't find shortest path because of an invalid move")
...
def main():
    ...
    if re.search('[FRBL]\d+', directions):
        ...
        for d in each_position:
            ...
            if (m[0] == "L" or m[0] == "R") and int(m[1]) >1:
                print("Invalid move, you cannot turn left or right more than once in one move.")
                final_directions = ""
                break
